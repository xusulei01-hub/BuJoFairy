...
class JournalViewModel: ObservableObject {
    ...
    func generateJournal(for trip: Trip, enableWebSearch: Bool = false, modelContext: ModelContext) async {
        guard AIService.shared.getAPIKey() != nil else {
            generationError = "请先在「我的」页面设置 Gemini API Key"
            return
        }
        ...
        let (systemPrompt, userPrompt) = JournalPromptBuilder.buildGenerationPrompt(
            tripName: trip.name,
        ...
        do {
            let rawContent = try await AIService.shared.chat(systemPrompt: systemPrompt, userMessage: userPrompt)
